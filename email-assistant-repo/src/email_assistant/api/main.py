...
from ..ml.classifier import load_model, predict as predict_local
...
@app.put("/taxonomy")
def put_taxonomy(payload: dict = Body(...)):
    taxonomy_store.write(payload); return payload
...
def put_rules(payload: dict = Body(...)):
    rules_store.write(payload); return payload
...
def graph_ingest(payload: dict = Body(...)):
...
def classify(payload: dict = Body(...)):
    ...
    if not os.path.exists(MODEL_PATH):
        raise HTTPException(status_code=400, detail="Model not trained. Run scripts/train_classifier.py")
